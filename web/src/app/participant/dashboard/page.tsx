    ...
    useEffect(() => {
        ...
        async function loadParticipantData() {
            try {
                ...
                if (userData.kind !== "participant") {
                    throw new Error("Access denied: Not a participant account.");
                }
                setUser(userData);
                ...
                if (sessionsRes?.ok) {
                    ...
                    setAvailableSessions(sessionsData.filter((s: any) => s.status === "CONFIRMED" || s.status === "SCHEDULED"));
                }
            } catch (err: any) {
                    ...
                    <div className="space-y-4">
                        {myBookings.length === 0 ? (
                            ...
                            myBookings.map((booking) => (
                                <div key={booking.id} className="border-2 border-[#171717] p-4 bg-[#FAF6EE] flex justify-between items-center shadow-[4px_4px_0_0_#171717]">
                                    <div>
                            ...
                            availableSessions.map((session) => (
                                <div key={session.id} className="border-2 border-[#171717] p-4 bg-[#FAF6EE] flex justify-between items-center shadow-[4px_4px_0_0_#171717]">
                                    ...
                                    <button
                                        ...
                                        className="border-2 border-[#171717] bg-[#2F4BFF] text-white px-4 py-2 text-xs font-bold uppercase shadow-[2px_2px_0_0_#171717] cursor-pointer disabled:opacity-50"
                                    >
                                        Book Place
                                    </button>
                                </div>
